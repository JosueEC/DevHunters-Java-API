...
public class UserController {
	...
	public ResponseEntity<ServerResponse> getUser (@PathVariable("userID") Long id) {
		return userService.findOneUser(id);
	}
	
	// * http://localhost:3001/api/v1/user [POST]
	@PostMapping
	public ResponseEntity<ServerResponse> postUser (@RequestBody User user) {
		return userService.insertUser(user);
	}
	
	// * http://localhost:3001/api/v1/user [PUT]
	@PutMapping
	public ResponseEntity<ServerResponse> putUser (@RequestBody User user) {
		return userService.updateUser(user);
	}
	
	// * http://localhost:3001/api/v1/user/:userID [DELETE]
	@DeleteMapping(path = "{userID}")
	public ResponseEntity<ServerResponse> deleteUser (@PathVariable("userID") Long id) {
